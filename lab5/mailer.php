...
if (strlen($username) <= 10) //if (!preg_match('/^[A-z0-9_\-]+[@][A-z0-9_\-]+([.][A-z0-9_\-]+)+[A-z.]{2,4}$/', $email))
{
...
if ($at === false) //if (!preg_match('/^([a-z0-9_\.-]+\@[\da-z\.-]+\.[a-z\.]{2,6})$/', $email))
...
if (strlen($message) < 25) //if (!preg_match('/^[A-z0-9_\-]+[@][A-z0-9_\-]+([.][A-z0-9_\-]+)+[A-z.]{2,4}$/', $email))
{
...
function show_error($myError)
{
    ...
    echo "
    <script type='text/javascript'>alert(' hello{$myError}');</script>
    <!-- <html>
    <body>

    <p>Please correct the following error:</p>
    <strong><?php /*echo $myError; */
    ?></strong>
